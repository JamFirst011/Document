...
2. 设备分配算法：FCFS，优先级
3. 安全性：
    - 安全分配：当进程请求设备后,若空闲就分配并继续运行；否则直接阻塞进程，降低并发度;设备要么持有所有需要的设备，要么就完全没有设备
...
**注意**：不同的IO控制方式有不同的驱动程序，并且驱动程序执行时程序不一定处于阻塞状态，例如DMA方式
...
4. 进程被唤醒
5. 等进程被调度后，由设备无关层程序(使用系统调用)将内核态数据复制到用户态中；
6. 进程从系统调用中返回，此时数据已经存储到变量中

**注意**：可以认为驱动程序是操作系统的一部分，但是设备无关层程序不算；因此综上，设备无关层是运行在用户态的，他主要的功能就是调用系统调用
